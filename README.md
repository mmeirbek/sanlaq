...
- **Шапка экрана**: инстанцируй `scenes/ui/components/sanlaq_header.tscn` и
  задай ему `kicker` / `title` / `subtitle`. Он сжимается по содержимому, так что
  длина строки и язык на раскладку не влияют. Не выкладывай заголовок
  отдельными Label поверх фона: фоны экранов — контрастные фотографии, и тёмный
  текст по ним не читается. Подложка решает это, не трогая ни фон, ни текст —
  приглушение фона убивает картинку, обводка по глифам выглядит наклейкой.
- **Кнопки**: обычная кнопка берёт стиль из темы, золотая «главная» — из
  вариации `PrimaryButton`. Не задавай `theme_override_styles/normal` поштучно:
  тогда у кнопки переопределён только один вид из пяти, и при наведении или
  нажатии она меняет цвет и размер невпопад.
